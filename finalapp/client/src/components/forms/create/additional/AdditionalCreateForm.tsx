...
export const AdditionalCreateForm = (props: AdditionalCreateFormProps): JSX.Element => {
    ...
    const DateToISOString = (date: string): string => {
        ...
        return new Date(Number(year), monthNumber, Number(day) + 1, Number(hours), Number(minutes)).toISOString()
    }
    ...
    return (
        <form className={styles.form}>
            <div className={styles.inputs}>
                ...
                <LabeledInput onChange={setPlace} type="text" label="Место проведения" />
                <ImageLoader onLoadCallback={(newImage) => setImage(newImage)} />
            </div>
            <div className={styles.buttons}>
                <Button callback={props.onCancel} type="default" text="Назад" />
                <Button callback={submitForm} type="primary" text="Далее" disabled={!checkForm()} />
            </div>
        </form>
    )
}
